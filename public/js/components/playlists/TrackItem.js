      ...
      <div 
        ...
        <div class="track-info ${draggable ? 'pl-6' : ''}">
          <div class="flex justify-between items-start gap-2">
            <div class="track-title font-medium text-sm line-clamp-1" title="${escapeHtml(track.title)}">${escapeHtml(track.title)}</div>
          ...
          <div class="track-meta text-xs text-muted mt-1">
            <div class="truncate">
              ${track.artist ? escapeHtml(track.artist) : ''}
              ${track.album ? `<span class="opacity-70"> • ${escapeHtml(track.album)}</span>` : ''}
            </div>
          </div>
        </div>
      </div>
