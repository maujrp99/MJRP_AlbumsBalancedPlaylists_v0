...
export class EditPlaylistView extends BaseView {
        ...
        <header class="view-header mb-8 fade-in">
          ...
          <div class="header-content mt-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            ...
            <div class="header-actions flex items-center gap-4">
              <span id="editStatusBadge" class="badge ${this.isLoading ? 'badge-warning' : 'badge-success'} px-3 py-1 rounded-full text-sm">
                ${this.isLoading ? 'EDITING: LOADING...' : `Editing: ${this.currentBatchName || 'Batch'}`}
              </span>
            </div>
          </div>
        </header>
  ...
  async mount(params) {
    // Extract batch name and series ID from URL query params
    // Router passes query string as params.query object
    const query = params.query || params
    this.originalBatchName = query.edit
    this.currentBatchName = query.edit
    this.seriesId = query.seriesId

    if (!this.originalBatchName) {
      toast.error('No batch name specified')
      router.navigate('/saved-playlists')
      return
    }
  ...
  async loadPlaylistsFromFirestore() {
    try {
      ...
      // Load into store (setPlaylists accepts seriesId as second param)
      ...
      <div class="playlist-card glass-panel rounded-xl p-4" data-playlist-index="${index}">
        <div class="flex items-center justify-between mb-3">
          <h3 class="text-lg font-bold flex items-center gap-2">
            <span class="text-accent-primary font-mono">${index + 1}.</span>
            ${getIcon('Music', 'w-5 h-5')} ${playlist.name}
          </h3>
          <div class="flex items-center gap-2">
            <span class="badge badge-sm">${playlist.tracks?.length || 0} tracks</span>
            <button class="btn btn-ghost btn-xs text-red-400 hover:bg-red-500/20" 
