...
- ✅ Mesmo `server/Dockerfile`
- ✅ Mesmo `scripts/deploy-backend.sh`
- ✅ Mesmas variáveis de ambiente
- ✅ Mesmo processo de deploy
...
# Deploy (COM BUILD)
npm run build           # ← NOVO STEP
firebase deploy --only hosting
```

**Resumo da mudança**:
1. ➕ Adicionar `npm run build` antes do deploy
2. 🔄 Mudar `"public": "public"` → `"public": "dist"`
3. ➕ Adicionar `rewrites` para SPA routing
...
# Build frontend
npm run build  # ← ÚNICA LINHA NOVA
...
**Mudanças**:
1. ➕ `npm ci` (instalar Vite)
2. ➕ `npm run build` (build frontend)

**Secrets**: ZERO novos secrets necessários
...
**Impacto**: 
- ✅ Firebase Hosting continua servindo
- ✅ URL continua a mesma
- ✅ Backend não afetado
- ✅ Rollback fácil (reverter deploy)
...
- Firebase Hosting: Spark Plan (grátis)
...
- Sem cache hash (pode servir versão velha)
- Sem minificação
...
### 1. "O que mudará nos deployments?"
...
**Sim!** Cloud Run não muda nada. Deploy backend continua:
```bash
...
### 4. "E variáveis de ambiente?"

**Backend**: Mesmas variáveis  
**Frontend**: Mesmas configurações (hard-coded, não secret)

**ZERO variáveis novas!**
...
1. **Desenvolver em branch** ✅ (já estamos em `feature/v2.0-foundation`)
...
**Benefícios**:
- ⚡ Dev experience muito melhor (HMR)
- 📦 Bundle otimizado (menor, mais rápido)
- 🎯 Code splitting automático
- 🔍 Source maps para debug

**Complexidade adicionada**:
- ➕ 1 linha no script de deploy
- ➕ 1 arquivo de config (vite.config.js)
- ➕ npm install Vite (1x)

**Trade-off**: Vale muito a pena! 🚀
...
**Deployment v2.0 é**:
- ✅ **Simples**: Apenas 1 build step novo
- ✅ **Seguro**: Rollback fácil
- ✅ **Sem custos**: $0 adicional
- ✅ **Sem configs**: ZERO variáveis novas
- ✅ **Backend inalterado**: Cloud Run continua igual
